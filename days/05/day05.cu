...
__global__ static void findLocationsRange(
        const size_t* seeds,
        const size_t length,
        const size_t* lengths,
        const size_t* sources,
...
void day05(const size_t part) {
  ...
  for (const std::string& line: lines) {
    if (line.starts_with("seeds: ")) {
      for (const std::string& e: splitString(line.substr(6), ' ')) {
  ...
  cudaMemcpy(
          deviceDestinations,
          destinations.data(),
          sizeof(size_t) * destinations.size(),
          cudaMemcpyHostToDevice
  );
  ...
  if (part == 1) {
    ...
    size_t seedRangeSize = 0;
    for (size_t i = 1; seeds.size() > i; i += 2) {
      seedRangeSize += (seeds.at(i) + 1);
    }
    size_t* deviceSeeds;
    cudaMalloc(&deviceSeeds, sizeof(size_t) * seeds.size());
    cudaMemcpy(
    ...
    size_t* deviceSeedRange;
    cudaMalloc(&deviceSeedRange, sizeof(size_t) * seedRangeSize);
    std::cout << "gets here 1" << seeds.size() << std::endl;

    for (size_t i = 0; seeds.size() > i; i++) {
      size_t range = 0;
      for (size_t j = range; (range + seeds[i]) >= j; j++) {
        cudaMemcpy(
                deviceSeedRange,
                seeds.data(),
                sizeof(size_t) * seeds.size(),
    ...
    size_t* deviceLocations;
    cudaMalloc(&deviceLocations, sizeof(size_t) * seedRangeSize);
    const dim3 block(seedRangeSize, 1, 1);
    findLocationsRange<<<grid, block, shared, stream>>>(
            deviceSeeds,
            lengths.size(),
            deviceLengths,
            deviceSources,
    ...
    size_t* locations = (size_t*)malloc(sizeof(size_t) * seedRangeSize);
    cudaMemcpy(
            locations,
            deviceLocations,
            sizeof(size_t) * seedRangeSize,
            cudaMemcpyDeviceToHost
    );
    size_t result = locations[0];
    for (size_t i = 0; seedRangeSize > i; i++) {
      if (result > locations[i]) {
        result = locations[i];
      }
      std::cout << locations[i] << std::endl;
