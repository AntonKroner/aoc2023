...
__global__ static void playGames(
        const size_t length,
...
__global__ static void findPowers(
        const size_t length,
        const size_t* lengths,
        const char* input,
        int* results
...
// This is a very stupid kernel that only computes the correct result
// *sometimes*. Pls help it get better!!
...
void two(const size_t part) {
  ...
  if (part == 1) {
    playGames<<<grid, block, shared, stream>>>(
            length,
            deviceLengths,
            deviceFlattened,
            games
    ...
    findPowers<<<grid, block, shared, stream>>>(
            length,
            deviceLengths,
            deviceFlattened,
            games
