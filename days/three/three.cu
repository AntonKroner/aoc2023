  ...
  for (size_t index = 1; (length - 1) > index; index++) {
    ...
    if (value) {
      if (rows[length + index] >= '0' && rows[length + index] <= '9') {
        if (isSymbol(rows[index - 1]) || isSymbol(rows[index])
            || isSymbol(rows[index + 1]) || isSymbol(rows[length + index - 1])
...
void three(const size_t part) {
  ...
  const ulong2 dimensions = make_ulong2(lines.at(0).size() + 2, lines.size() + 2);
  ...
  cudaMemcpy(
          deviceSchematic,
          schematic.data(),
          sizeof(char) * (dimensions.x) * (dimensions.y),
