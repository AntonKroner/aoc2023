...
void one(const size_t part) {
	...
	if (part == 1) {
		calibrate1 << <grid, block, shared, stream >> > (length, deviceLengths, deviceFlattened, calibrations);
		std::cout << "part 1" << std::endl;
	}
	else {
		calibrate2 << <grid, block, shared, stream >> > (length, deviceLengths, deviceFlattened, calibrations);
